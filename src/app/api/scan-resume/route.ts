...
import type { ApiResponse, ResumeLiteResultData } from "@/types";
import fs from "fs";
...
async function parseFormData(
  req: NextRequest,
): Promise<{ file: File; email: string }> {
  try {
...
function getMockResponse(): ResumeLiteResultData {
  try {
...
export async function POST(req: NextRequest) {
  try {
      ...
      return NextResponse.json<ApiResponse>(
        {
          success: false,
          error:
            "Invalid request format. Please ensure you are uploading a file properly.",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      console.warn(
        "Failed to store email, continuing with analysis:",
        emailError,
      );
    }
    ...
    if (process.env.DEVELOPMENT_MODE === "true") {
        ...
        return NextResponse.json<ApiResponse>(
          {
            success: false,
            error:
              "Failed to load development test data. Please check if resume-analysis.json exists in the public folder.",
          },
          { status: 500 },
        );
      }
      ...
      try {
        ...
        if (
          ...
          statusCode = 503;
          userMessage =
            "Service configuration issue: OpenAI API key is missing or invalid. Please contact support or check your environment configuration.";
        } else if (
          errorMessage.includes("Assistant ID") ||
          errorMessage.includes("OPENAI_ASSISTANT_ID")
        ) {
          statusCode = 503;
          userMessage =
            "Service configuration issue: OpenAI Assistant is not properly configured. Please contact support or check your environment configuration.";
        } else if (
          ...
          statusCode = 429;
          userMessage =
            "Service is currently experiencing high demand. Please try again in a few minutes.";
        } else if (
          ...
          statusCode = 400;
          userMessage =
            "The uploaded file format is not supported. Please upload a PDF, DOCX, or TXT file.";
        }
