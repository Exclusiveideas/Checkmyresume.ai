...
function validateEnvironment(): { isValid: boolean; error?: string } {
  if (!process.env.OPENAI_API_KEY) {
    return {
      isValid: false,
      error:
        "OpenAI API key is missing. Please add OPENAI_API_KEY to your .env.local file. Get your API key from https://platform.openai.com/api-keys",
    };
  }
  ...
  if (!process.env.OPENAI_ASSISTANT_ID) {
    return {
      isValid: false,
      error:
        "OpenAI Assistant ID is missing. Please add OPENAI_ASSISTANT_ID to your .env.local file. Create an assistant at https://platform.openai.com/assistants",
    };
  }
...
export async function analyzeResume(
  ...
  if (fileSize > OPENAI_MAX_FILE_SIZE) {
    throw new Error(
      `File size (${(fileSize / 1024 / 1024).toFixed(2)}MB) exceeds OpenAI's maximum limit of 512MB`,
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        errorWithMessage?.message?.includes("413")
      ) {
        // If file is too large, try to extract text and send as message instead
        return await analyzeResumeAsText(
          fileBuffer,
          fileName,
          openai,
          ASSISTANT_ID,
...
The resume has been uploaded as a file attachment.`,
        ...
        if (retryCount >= maxRetries) {
          ...
          try {
            ...
            console.warn(
              "Failed to delete uploaded file after thread creation failure:",
              cleanupError,
            );
          }
          throw new Error(
            `Failed to create thread after ${maxRetries} attempts: ${threadError instanceof Error ? threadError.message : "Unknown error"}`,
          );
        }
    ...
    while (
      ...
      if (Date.now() - startTime > maxWaitTime) {
        ...
        throw new Error(
          "Analysis timeout - the file may be too complex or large",
        );
      }
    ...
    if (runStatus.status === "completed") {
      ...
      if (response.content[0].type === "text") {
        ...
        try {
          ...
          const analysis = JSON.parse(jsonMatch[0]) as ResumeLiteResultData;
...
async function analyzeResumeAsText(
  ...
  try {
    ...
    if (fileName.toLowerCase().endsWith(".pdf")) {
      ...
      const textMatches = pdfText.match(
        /(?:BT\s*\/F\d+\s+\d+\s+Tf\s*)(.*?)(?:ET)/g,
      );
    ...
    if (!resumeText || resumeText.length < 50) {
      throw new Error(
        "Unable to extract sufficient text from the file. Please ensure it contains readable text.",
      );
    }
...
${resumeText}`,
    ...
    throw new Error(
      `Analysis failed: ${runStatus.last_error?.message || "Unknown error"}`,
    );
  } catch (error) {
    throw error;
  }
